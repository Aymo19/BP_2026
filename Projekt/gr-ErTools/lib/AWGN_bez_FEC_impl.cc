...
AWGN_bez_FEC_impl::AWGN_bez_FEC_impl(int M)
    ...
    R_B(rd_B()), // Inicializacia random seed
    Gauss_B(0,1) // Tvoba normalneho rozdelenia N(0,1)
{
                            ...
                            gr_vector_void_star& output_items)
{
    ...
    // Vypocet vykonu vstupneho signalu Es
    ...
    for(int b = 0; b < noutput_items; b++) { 
      EBQ_B = in1[b]; // index EbN0 z Master Clock
    
      // Vypocet sumu
      gr_complex sg_n = Sum_B(EBQ_B, _M, Es);
      
      // Vyslanie sumu na vystup
      out[b] = sg_n; 
    }
